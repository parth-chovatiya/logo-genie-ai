...
export async function generateLogos(params: LogoGenerationParams): Promise<GeneratedLogo[]> {
...
The logo should be centered on a square canvas with adequate padding around the design.`;
  ...
  if (logos.length === 0) {
    throw new Error("Failed to generate any logos. Please check your API key and try again.");
  }
